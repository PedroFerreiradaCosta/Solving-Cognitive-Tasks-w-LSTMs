...
def main(is_2layers, root_logdir, load_data):
	"""
	...
	if load_data:
		...
		data = cdt.create_data(task = 'None', plot = False, trials = 2000)
	...
	### defines model
	model = Sequential()
	model.add(LSTM(h_units1, input_shape=(X_training.shape[1], X_training.shape[2]),return_sequences=True))#, return_state= True))
	if is_2layers:
